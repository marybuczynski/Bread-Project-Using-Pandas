...
TOKEN = 'rXvOQ7sO1qh6ORLIdzIo5rDO8gfG1eLSC6gtfpsX'
...
def get_food_ids(searchterm, num_results):
    ...
    url = "https://api.nal.usda.gov/fdc/v1/foods/search?api_key=" + TOKEN + "&query=" + searchterm.replace(" ", "%20") + "&pageSize=" + str(num_results)
...
def get_details_by_id(ids):
    ...
    url = "https://api.nal.usda.gov/fdc/v1/foods?fdcIds=" + '&fdcIds='.join(ids) + "&api_key=" + TOKEN
    headers = { 'Content-Type': 'application/json' }  
    parameters = {}
    response = requests.get(url, params=parameters, headers=headers)
    data = json.loads(response.text)
